...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    bs, num_heads, seq_len, head_dim = context_layer.size()
    context_layer = context_layer.transpose(1, 2).reshape(bs, seq_len, num_heads * head_dim) 
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    ### TODO
    hidden_states_out = self.self_attention(hidden_states, attention_mask)
    h_prime = self.add_norm(hidden_states, hidden_states_out, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    #feed_forward
    ffn = self.interm_af(self.interm_dense(h_prime))
    out = self.add_norm(h_prime, ffn, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    combined = inputs_embeds + pos_embeds + tk_type_embeds
    combined = self.embed_layer_norm(combined)
